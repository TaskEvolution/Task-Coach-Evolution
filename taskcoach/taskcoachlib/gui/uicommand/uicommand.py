    ...
    def doCommand(self, event):
        ...
        for task in tasklist:
            ...
            if not exists:
                ...
                if category is None:
                    newCategoryCommand = command.NewCategoryCommand(self.mainWindow().taskFile.categories(),
...
class QuickAdd(TaskListCommand, ViewerCommand, settings_uicommand.SettingsCommand):
    ...
    def onEnterKey(self, event):
        if event.KeyCode == wx.WXK_RETURN:
            ...
            newTaskCommand = command.NewTaskCommand(self.mainWindow().taskFile.tasks(),
                                                    subject=taskArgs['Title'], description=taskArgs['Description'],
                                                    plannedStartDateTime=taskArgs['StartDate'],
                                                    dueDateTime=taskArgs['EndDate'],
                                                    priority=taskArgs['Priority'],
                                                    actualStartDateTime=taskArgs['ActualStartDate'],
